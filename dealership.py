...
def login():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        cur.execute('SELECT * FROM customer_login WHERE email = %s AND password = %s', (username, password,))
        ...
        if customer_login:
           ...
           # msg = 'Wrong username/password'
            ...
            cur1.execute('SELECT * FROM employee_login WHERE email = %s and password = %s', (username, password,))
            employeelogin = cur1.fetchone()
            if employeelogin:
             session['loggedin'] = True
             session['employee_id'] = employeelogin['employee_id']
             session['email'] = employeelogin['email']
...
def favorites():
     if 'username' in session:  
        ...
        if customer_login:
            ...
            cur1 = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
            cur1.execute('SELECT * FROM employee_login WHERE email = %s',(userEmail,))
