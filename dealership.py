...
def employeeVehicleAddCarPage():
    ...
    if request.method == 'POST' and 'make' in request.form  and 'color' in request.form:
       ...
       cursor = mysql.connection.cursor()
       cursor.execute('INSERT INTO inventory newVin = %s make = %s, model = %s, year = %s, color = %s, mileage = %s, price = %s, our_price = %s, transmission = %s, body_style = %s, car_photo = %s WHERE  vin = %s', (newVin, make, model, year, color, mileage, price, our_price, transmission, body_style, car_photo,))
       mysql.connection.commit()
